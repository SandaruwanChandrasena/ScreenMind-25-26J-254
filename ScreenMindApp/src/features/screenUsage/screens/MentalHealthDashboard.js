...
export default function MentalHealthDashboard({ route, navigation }) {
  ...
  // ---------- 7-day usage aggregates + chart data ----------
  const usageSummary = useMemo(() => {
    if (!usageWindow.length) return null;

    // Your generator is newest first; for charts, we want oldest -> newest left-to-right.
    ...
    const avgTotalMin = Math.round(avg(total));
    const avgNightMin = Math.round(avg(night));
    const avgUnlocks = Math.round(avg(unlock));
    const avgSessionMin = Math.round(avg(session));
  ...
  const trend = useMemo(() => {
    if (usageSummary?.newestRisk === null || usageSummary?.newestRisk === undefined) return null;
    if (usageSummary?.oldestRisk === null || usageSummary?.oldestRisk === undefined) return null;
  ...
  const keyInsights = useMemo(() => {
    ...
    if (newest) {
      if ((newest.nightUsageMin ?? 0) >= 90) list.push({ icon: "🌙", text: "High night-time phone use." });
      if ((newest.totalScreenTimeMin ?? 0) >= 360) list.push({ icon: "📱", text: "High daily screen time." });
      if ((newest.unlockCount ?? 0) >= 120) list.push({ icon: "🔓", text: "Very frequent checking (many unlocks)." });
      if ((newest.avgSessionMin ?? 0) >= 20) list.push({ icon: "⏳", text: "Long sessions (possible doom-scrolling)." });
    }
    ...
    if (!list.length) list.push({ icon: "ℹ️", text: "Not enough signals yet for strong insights." });

    return list.slice(0, 6);
  }, [usageSummary, phqScore, gadScore]);
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.subtitle}>Based on your check-in answers and recent phone usage patterns.</Text>
        ...
        <View style={styles.summaryCard}>
          ...
          {submittedAt ? <Text style={styles.smallMuted}>Last updated: {formatDateTime(submittedAt)}</Text> : null}
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          {trend ? (
            ...
            <Text style={styles.muted}>Trend needs a 7-day window to compare.</Text>
          )}
        </View>
        ...
        {/* Usage card */}
        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>Phone Usage (Last 7 Days)</Text>
          ...
          {usageSummary ? (
            <>
              ...
              <Text style={styles.miniTitle}>Weekly pattern (simple chart)</Text>

              <MiniBarChart
                title="📱 Screen time"
                unit="min"
                values={usageSummary.chart.total}
                labels={usageSummary.chart.labels}
              />

              <MiniBarChart
                title="🌙 Night usage"
                unit="min"
                values={usageSummary.chart.night}
                labels={usageSummary.chart.labels}
              />
              ...
              <MiniBarChart
                title="🔓 Unlocks"
                unit="times"
                values={usageSummary.chart.unlock}
                labels={usageSummary.chart.labels}
              />
              ...
              <MiniBarChart
                title="⏳ Avg session"
                unit="min"
                values={usageSummary.chart.session}
                labels={usageSummary.chart.labels}
              />
            </>
          ) : (
            <Text style={styles.muted}>No usage window found in this result.</Text>
          )}
...
function computeDayUsageRisk01(day) {
  ...
  const risk = 0.4 * screenTimeScore + 0.3 * nightScore + 0.2 * unlockScore + 0.1 * sessionScore;
  return clamp01(risk);
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: colors.muted, lineHeight: 18, marginBottom: spacing.lg },
