...
// ✅ import both getUsageStats AND getUnlockCount
import { getUsageStats, getUnlockCount } from "../services/usageStatsNative";
...
export default function QuestionnaireScreen({ navigation }) {
  ...
  async function onSubmit() {
    ...
    try {
      ...
      try {
        // ✅ Fetch both usage stats AND real unlock count in parallel
        const [rawUsage, realUnlockCount] = await Promise.all([
          getUsageStats(),
          getUnlockCount(),
        ]);

        console.log("[Questionnaire] Real unlock count:", realUnlockCount);

        const features     = extractUsageFeatures(rawUsage);

        // ✅ Pass real unlockCount into the adapter
        const realUsageLog = adaptRealUsageToModel({
          ...features,
          unlockCount: realUnlockCount > 0 ? realUnlockCount : undefined,
        });
        ...
        if (hasRealUsage) {
          ...
          usageSource       = "real";
          realUsageFeatures = { ...features, unlockCount: realUsageLog.unlockCount };
        } else {
          usageWindow = generateSimulatedUsageWindow(7, { phq9Score, gad7Score });
        }

      } catch (e) {
        console.log("[Questionnaire] Real usage fetch failed, using simulated:", e);
        usageWindow = generateSimulatedUsageWindow(7, { phq9Score, gad7Score });
      }
